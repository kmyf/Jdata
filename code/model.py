...
def fit_predict_1(X, y, X_pred):
    ...
    for k, (train_index, test_index) in enumerate(SK.split(X, y)):
        ...
        gbm = lgb.train(param_1, lgb_train, num_boost_round=num_boost_round, valid_sets=lgb_eval,
                        early_stopping_rounds=early_stopping_rounds)
        stacking_model.append(gbm)
        layer_train[:, 1][test_index] = gbm.predict(X_test)
    ...
    for bn in range(bagging_num):
        ...
        gbm = lgb.train(param_1, lgb_train, num_boost_round=5000, valid_sets=lgb_eval, early_stopping_rounds=200)
...
def fit_predict_2(X, y, X_pred):
    ...
    for k, (train_index, test_index) in enumerate(SK.split(X, y)):
        ...
        gbm = lgb.train(param_2, lgb_train, num_boost_round=num_boost_round, valid_sets=lgb_eval,
                        early_stopping_rounds=early_stopping_rounds)
        stacking_model.append(gbm)
        layer_train[:, 1][test_index] = gbm.predict(X_test)
    ...
    for bn in range(bagging_num):
        ...
        gbm = lgb.train(param_2, lgb_train, num_boost_round=5000, valid_sets=lgb_eval, early_stopping_rounds=200)
...
def fit_predict_1(X, y, X_pred):
    ...
    gbm = lgb.train(param_1, lgb_train, num_boost_round=3000, valid_sets=lgb_eval, early_stopping_rounds=200)

    # X_pred = X_pred.values
    # lgb_train = lgb.Dataset(X, y)
    # gbm = lgb.train(param_1, lgb_train, num_boost_round=200)
...
def fit_predict_2(X, y, X_pred):
    ...
    gbm = lgb.train(param_2, lgb_train, num_boost_round=3000, valid_sets=lgb_eval, early_stopping_rounds=200)

    # X_pred = X_pred.values
    # lgb_train = lgb.Dataset(X, y)
    # gbm = lgb.train(param_2, lgb_train, num_boost_round=200)
